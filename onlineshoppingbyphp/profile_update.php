<?php 
	...
	$sql="SELECT users SET name=:value1, profile=:value2, email=:value3, password=:value4, phone=:value5, address=:value6, status=:value7 WHERE id=:value8";
	...
	$sql="SELECT users.*, roles.name as rname FROM users INNER JOIN model_has_roles ON users.id=model_has_roles.user_id INNER JOIN roles ON model_has_roles.role_id=roles.id WHERE users.id=:v1";

	$stmt=$conn->prepare($sql);
	$stmt->bindParam(':v1',$id);
	$stmt->execute();
	$user=$stmt->fetch(PDO::FETCH_ASSCO);
